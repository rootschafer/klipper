# Virtual heater bed that works with virtual thermistor sensors
#
# This module provides a heater bed implementation that can work with
# virtual temperature sensors instead of requiring physical sensors.
#
# Usage:
#   [virtual_heater_bed]
#   heater_pin: PD4
#   sensor: v_bed          # reference to temperature_sensor section
#   control: watermark
#   min_temp: 0
#   max_temp: 130
#
...
class VirtualHeaterBed:
    def __init__(self, config):
        self.printer = config.get_printer()
        self.name = config.get_name()

        # Get heater configuration
        self.heater_pin = config.get('heater_pin', None)  # Optional for virtual heaters
        self.max_power = config.getfloat('max_power', 1.0)
        self.min_temp = config.getfloat('min_temp', 0)
        self.max_temp = config.getfloat('max_temp', 130)
        ...
        # Setup heater control (virtual - no physical pin needed)
    ...
    def _handle_connect(self):
        ...
        if self.sensor is None:
            try:
                ...
                # Verify it's a virtual thermistor
                if not hasattr(sensor_obj, 'get_temp') or not hasattr(sensor_obj, 'get_status'):
                    raise Exception(f"'{self.sensor_name}' is not a valid temperature sensor")
...
def load_config(config):
    return VirtualHeaterBed(config)
